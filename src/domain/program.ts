 ...
 */
export const programs: NonEmpty<Program> = [
  pelvicFloorProgram,
  coreProgram,
  birthPrepProgram,
];
...
export const PROGRAM_IDS: NonEmpty<ProgramId> = [
  "pelvic-floor",
  "core",
  "birth-prep",
];
...
/** Deep stretch becomes "Recovery stretches" postpartum; the rest keep one name. */
export function programTitle(program: Program, phase: Phase): string {
  return phase === "postpartum"
    ? (program.postpartumTitle ?? program.title)
    : program.title;
}
...
export function stagesForProgram(programId: ProgramId, phase: Phase): Stage[] {
  return programsById[programId].stages.filter(
...
export function stageFor(
  ...
  if (!first || !last)
    throw new Error(`No stages defined for ${programId} in phase: ${phase}`);

  const match = candidates.find(
    (stage) =>
      week >= stage.startWeek &&
      (stage.endWeek === null || week <= stage.endWeek),
  );
...
export function stagesForWeek(
  phase: Phase,
  week: number,
): Record<ProgramId, Stage> {
  return {
