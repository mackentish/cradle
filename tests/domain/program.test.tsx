...
describe("the exercise library", () => {
  ...
  it("reports nothing for an id that came from outside the type system", () => {
    ...
    expect(findExercise("diaphragmatic-breath")?.name).toBe(
...
describe("the program registry", () => {
  ...
  it("gives every program a complete control tone, all three distinct", () => {
    ...
    const fills = PROGRAM_IDS.map(
      (id) => programColors[programsById[id].colorKey].ring,
    );
  ...
  it("agrees on week boundaries across programs, so one banner speaks for all", () => {
    for (const stageId of programsById["pelvic-floor"].stages.map(
      (s) => s.id,
    )) {
  ...
  it("tags every stage with the program that owns it", () => {
    for (const id of PROGRAM_IDS) {
      for (const stage of programsById[id].stages)
        expect(stage.programId).toBe(id);
    }
  });

  it("retitles deep stretch postpartum, and leaves the others alone", () => {
    expect(programTitle(programsById["birth-prep"], "pregnancy")).toBe(
      "Deep stretch",
    );
    expect(programTitle(programsById["birth-prep"], "postpartum")).toBe(
      "Recovery stretches",
    );
    expect(programTitle(programsById.core, "postpartum")).toBe("Core strength");
    expect(programTitle(programsById["pelvic-floor"], "postpartum")).toBe(
      "Pelvic floor",
    );
  });
  ...
  it("gives every session a globally unique id", () => {
    ...
    const ids = stages.flatMap((stage) =>
      stage.sessions.map((session) => session.id),
    );
  ...
  it("gives every session in a stage band a distinguishable title", () => {
    for (const stageId of programsById["pelvic-floor"].stages.map(
      ...
      const titles = PROGRAM_IDS.flatMap(
        (id) =>
          stageById(id, stageId)?.sessions.map((session) => session.title) ??
          [],
      );
...
describe("the daily session rotation", () => {
  ...
  it("gives the three programs different sessions on the same day", () => {
    ...
    const picked = PROGRAM_IDS.map((id: ProgramId) => {
      ...
      return {
        id,
        letter: sessionForDay(stage).id.slice(-1),
        count: stage.sessions.length,
      };
    });
