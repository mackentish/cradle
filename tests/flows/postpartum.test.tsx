...
describe("the postpartum switch", () => {
  it("moves the whole program to recovery once a birth date is set", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText("Baby has arrived")).toBeOnTheScreen(),
    );
    ...
    await waitFor(() =>
      expect(screen.getByText("Postpartum program")).toBeOnTheScreen(),
    );
    ...
    // Deep stretch retitles postpartum rather than lingering under a labor name. Its
    ...
    expect(screen.queryByText("Deep stretch")).not.toBeOnTheScreen();
  });
});
