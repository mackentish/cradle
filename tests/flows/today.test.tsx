...
describe("Today", () => {
  ...
  it("keeps each card collapsed until it is tapped", async () => {
    ...
    fireEvent.press(within(card).getByLabelText(/^Core strength,/));

    await waitFor(() =>
      expect(screen.getByTestId("program-steps-core")).toBeOnTheScreen(),
    );
    // The other two stay closed — expanding is per card.
    expect(
      screen.queryByTestId("program-steps-pelvic-floor"),
    ).not.toBeOnTheScreen();
  });
  ...
  it("drops the steps from the tree once a card has closed again", async () => {
    ...
    const card = screen.getByTestId("program-card-core");
    const header = within(card).getByLabelText(/^Core strength,/);
